...
jQuery.reel || (function($, window, document, undefined){
    ...
    $.fn.reel = function(options)
    {
                    ...
                    on= {
                        ...
                        */
                        ...
                        preload: function(e)
                        {
                            ...
                            {
                                var uri  = opt.path+preload.shift();
                                    ...
                                    .bind(
                                        ...
                                        function update_preloader()
                                        {
                                            ...
                                            if (img_tag.frames == img_tag.preloaded)
                                            {
                                                ...
                                                images.length || 
                                                t.css({
                                                    backgroundImage: url(opt.path+image)
                                                });
                        ...
                        tick: function(e)
                        {
                            ...
                            if (operated && !velocity)
                            {
                                return cleanup.call(e);
                            }
                            
                            if (get(_clicked_))
                            {
                                return cleanup.call(e, unidle());
                            }
                        ...
                        pause: function(e)
                        {
                            var playing = set(_playing_, false);
                        ...
                        {                        
                            ...
                            */
                            if (opt.draggable && slidable)
                            {
                                ...
                                var delta = { x: x - last.x, y: y - last.y };
                                ...
                                if (abs(delta.x) > 0 || abs(delta.y) > 0)
                                {
                                    ...
                                    var origin= !(fraction % 1) && !opt.loops && recenter_mouse(x, y, fraction, revolution, get(_row_))
                                    t.trigger('fractionChange');
                                }
                            }
                        ...
                        wheel: function(e, distance)
                        {
                            ...
                            // detects motion direction, nullifies the velocity
        ...
        ticker = ticker || (
            function tick()
            {
                var start = +new Date(), tempo = leader(_tempo_);
    ...
    function leader(key)
    {
        return $.reel.instances.length ? $.reel.instances.first().data(key) : null
    }

    $.reel.leader = leader;

    function add_instance($instance)
    {
        return ($.reel.instances.push($instance[0])) && $instance;
    }

    function remove_instance($instance)
    {
        return ($.reel.instances= $.reel.instances.not('#'+$instance.attr(_id_))) && $instance;
    }
    ...
    // Helpers
    function embedded(image)
    {
        return knows_data_url && 'data:image/gif;base64,R0lGODlh'+image;
    }
    ...
    function cdn(path)
    {
        return 'http://code.vostrel.cz/' + path;
    }
